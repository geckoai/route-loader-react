    ...
    Preloaded.prototype.useState = function () {
        ...
        return [this.data, function (value) {
                if (typeof value === 'function') {
                    navigate('?' + qs.stringify(__assign(__assign({}, _this.originParams), value(_this.params)), {
                        serializeDate: function (d) { return String(d.getTime()); }
                    }));
                }
                else {
                    navigate('?' + qs.stringify(__assign(__assign({}, _this.originParams), value), {
                        serializeDate: function (d) { return String(d.getTime()); }
                    }));
                }
            }];
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        query = qs.parse(url.search.replace(/^\?/, ''), RouteLoader.options);
                        transformer = container.get(ClassTransformer);
